...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    ...
    std::unique_ptr<TrieNode> tmp(new TrieNode(std::move(*child)));
//    std::unique_ptr<TrieNode> res = tmp;
    children_[key_char] = std::move(tmp);
...
class Trie {
 ...
 public:
   ...
   */
  Trie() {
    std::unique_ptr<TrieNode> tmp(new TrieNode('\0'));
    root_ = std::move(tmp);
  }
  ...
  bool Insert(const std::string &key, T value) {
    ...
    if (child == nullptr) {
      std::unique_ptr<TrieNode> tmp(new TrieNodeWithValue(key[i], value));
      (*cur)->InsertChildNode(key[i], std::move(tmp));
    } else if ( !(*child)->IsEndNode()) {
      /*
      * 2. If the terminal node is a TrieNode, then convert it into TrieNodeWithValue by
      * invoking the appropriate constructor
      * */
      (*child) = std::make_unique<TrieNodeWithValue<T>>(std::move(*(*cur)), value);
    } else {
      return false;
    }
  ...
  bool Remove(const std::string &key) {
    ...
    remove_inner(key, 0, cur, success);
    return *success;
  }
